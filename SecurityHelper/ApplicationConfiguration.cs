    ...
    public class ApplicationConfiguration
    {
        ...
        public static string Writer(string plainText)
        {
            ...
            byte[] keyBytes = new Rfc2898DeriveBytes(
                _hasher, Encoding.ASCII.GetBytes(_saltPepper)).GetBytes(256 / 8);

            var symmetricKey = new RijndaelManaged() { Mode = CipherMode.CBC, Padding = PaddingMode.Zeros };

            var encryptor = symmetricKey.CreateEncryptor(
                keyBytes, Encoding.ASCII.GetBytes(_mainKeyer));
        ...
        public static string Reader(string encryptedText)
        {
            ...
            byte[] keyBytes = new Rfc2898DeriveBytes(
                _hasher,
                Encoding.ASCII.GetBytes(_saltPepper)).GetBytes(256 / 8);
